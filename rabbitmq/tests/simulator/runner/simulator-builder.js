...
builder.options('messageId', utils.create_hash().toString());
builder.options('sessionId', utils.create_hash().toString());
builder.options('exchange', 'service-a.tasks.exchange');
builder.options('route', 'service-a.tasks.exchange', 'tasks.service-a');
builder.options('type', 'task');
builder.options('body', 'task message from service-a');
builder.options('options', builder.getOptions());
...
messageId:uuid,sessionId:uuid,exchange:string,route:string,type:string,body:string\(1024\),options:json,linked:uuid,owner:string
